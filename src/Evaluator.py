...
def cPSNR(sr, hr, hr_map):
    ...
    if sr.dtype.type is np.uint16:  # integer array is in the range [0, 65536]
        ...
        # means that we allow negative values
        assert -10 <= sr.min() and sr.max() <= 10
...
def shift_cPSNR(sr, hr, hr_map, border_w=3):
    ...
    site_cPSNR = np.array([cPSNR(sr, hr, hr_map) for hr, hr_map in tqdm(zip(iter_hr, iter_hr_map), disable=(len(sr.shape) == 2))])
